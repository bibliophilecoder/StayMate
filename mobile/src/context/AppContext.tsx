...
export function AppProvider({children}:PropsWithChildren){
 ...
 useEffect(()=>{Promise.all([AsyncStorage.getItem("onboarded"),SecureStore.getItemAsync("staymate_token")]).then(([seen,token])=>{setOnboarded(seen==="true");setAuthenticated(Boolean(token));setReady(true);if(token)void refresh()})},[refresh]);
 const completeOnboarding=()=>{setOnboarded(true);void AsyncStorage.setItem("onboarded","true")};
 const swipe=async(liked:boolean)=>{const profile=profiles[index%profiles.length];if(!profile)return;setIndex(x=>x+1);if(liked)setLikes(x=>[...new Set([...x,profile.id])]);if(authenticated)try{await swipeProfile(profile.id,liked)}catch(e){setError(e instanceof Error?e.message:"Swipe failed")}};
